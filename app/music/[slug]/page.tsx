import SimilarMusic from "@/components/music/similar-music";
...
const MusicDetail = ({ params: { slug } }: { params: { slug: string } }) => {
  const selectedMusic = music.find((music) => music.slug === slug) as Music;
  ...
  return (
    <main className="pt-28">
      <section
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4)), url(${selectedMusic?.poster})`,
      ...
      >
        <div className="container">
          <Link className=" flex gap-2 mb-10 text-white" href="/music">
          ...
          <div className="mb-16 grid grid-cols-1 md:grid-cols-2 items-center relative z-10">
            <div className="relative rounded-lg overflow-hidden">
              <Image
                src={selectedMusic?.poster || "/default-poster.png"}
            ...
            <div className=" space-y-4 xl:ml-[-340px] lg:ml-[-150px] sm:ml-0 md:ml-0 mb-10">
              <h1 className="text-3xl font-bold text-white">{selectedMusic.title}</h1>
              <div className="flex items-center gap-2">
                <StarIcon className="w-5 h-5 fill-yellow-400 text-yellow-400" />
                <span className="text-white">{selectedMusic.rating}</span>
              </div>
              <div className="flex items-center gap-2 text-white">
                <ListIcon className="w-5 h-5" />
                <span>{selectedMusic.categories.join(",")}</span>
              </div>
              <div className="flex items-center gap-2 text-white">
      ...
      <div className="container">
        <div>
          <h2 className="font-bold text-3xl mb-16">O&apos;xshash Music</h2>

          <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
            {music.map((music) => (
              <SimilarMusic key={music.id} {...music} />
            ))}
          </section>
        </div>
      </div>
