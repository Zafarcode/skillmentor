...
export default function MusicCard({
  ...
  // Favoritlar holatini saqlash uchun state
  ...
  const toggleFavorite = () => {
    ...
    if (isFavorite) {
      // Agar music favorit bo'lsa, uni ro'yxatdan o'chirish
      const updatedFavorites = favorites.filter((s: string) => s !== slug);
      localStorage.setItem("favorites", JSON.stringify(updatedFavorites));
    } else {
      // Agar music favorit bo'lmasa, uni ro'yxatga qo'shish
      favorites.push(slug);
  ...
  return (
    <Card className="w-full h-full max-w-sm overflow-hidden shadow-none border-none rounded-t-lg rounded-b-none bg-transparent">
      <CardContent className="p-0">
        <Link className="relative group" href={"/music/" + slug}>
          <Image
            src={poster}
            alt={title}
            width={500}
